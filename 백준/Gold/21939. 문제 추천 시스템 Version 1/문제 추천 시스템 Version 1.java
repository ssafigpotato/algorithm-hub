...
public class Main {
	...
	static class Problem implements Comparable<Problem>{
		...
		public boolean equals(Object obj) {
			// 객체 참조 비교:
			// 현재 객체(this)와 비교 대상 객체(obj)가 같은 참조를 가리키는 경우, 바로 true
			// 두 객체가 같은 메모리 주소를 참조하면, 이는 곧 두 객체가 완전히 동일하다는 것을 의미
			...
			// 잘못된 타입의 객체와 비교하는 것을 방지하여 ClassCastException과 같은 런타임 오류를 피함
		...
		public int hashCode() {
			...
			// equals에서 동일하다고 판단한 객체는 반드시 동일한 해시코드를 가져야
	...
	public static void main(String[] args) {
		...
		// removeIf 에서 연산이 많이 일어나서 시간초과 됨
		// 방지를 위해 hashmap도 함께 사용 
		...
		for(int i = 0; i <M; i++) {
			...
			if(command.equals("recommend")) {
				...
				// remove 메소드는 P뿐만 아니라 난이도 L도 알아야함 
